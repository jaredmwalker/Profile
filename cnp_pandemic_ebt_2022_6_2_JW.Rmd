...
```{r}
r3 <- read_csv("P:/1_Internal/Reports/CNP (all programs)/COVID-19/PEBT SY2022/SFA Submissions SY2022/1_PEBT 3/P_EBT SY22 Round 3 DWS Submission 20220630_Prelim.csv") %>% clean_names %>%
...
pebt_files <- list.files(path = "P:/1_Internal/Reports/CNP (all programs)/COVID-19/PEBT SY2022/SFA Submissions SY2022/1_PEBT 3/Data Files//", pattern = "*csv", full.names = TRUE)
...
pebt_super2 <- pebt_super %>%
  mutate(na_count = rowSums(!is.na(pebt_super)), .before = lea_code) %>%
  filter(na_count > 1) %>%
  select(-na_count)
...
#create flag for duplicate SSIDs WITHIN LEAS
pebt_super3 <- pebt_super2 %>% 
  group_by(student_ssid) %>%
  mutate(duplicate_flag = +(n() > 1)) %>%
  relocate(duplicate_flag) %>%
  ungroup()

lea_duplicates <- pebt_super2 %>% 
  group_by(lea_name,student_ssid) %>%
  mutate(duplicate_flag = +(n() > 1)) %>%
  relocate(duplicate_flag) %>%
  ungroup() 
...
View(lea_duplicates)
lea_duplicates <- lea_duplicates %>%
  filter(duplicate_flag > 0)

write.xlsx(lea_duplicates, "P:/1_Internal/Reports/CNP (all programs)/COVID-19/PEBT SY2022/SFA Submissions SY2022/2_Summer Round 1/lea_duplicates.xlsx", 
             overwrite = F)
#check to see if duplicates are within the same LEA or across LEAs (none of the duplicates are within the same LEA)

all_duplicates <- pebt_super2 %>% 
  group_by(lea_name,student_ssid) %>%
  mutate(duplicate_flag = +(n() > 1)) %>%
  relocate(duplicate_flag) %>%
  ungroup()
...
write.xlsx(all_duplicates, "P:/1_Internal/Reports/CNP (all programs)/COVID-19/PEBT SY2022/SFA Submissions SY2022/2_Summer Round 1/all_duplicates.xlsx", overwrite = T)
