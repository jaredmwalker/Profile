---
title: <center>FNS 905 Summer Site-Finder Report</center>
subtitle:
output:
  html_document: 
    toc: yes
  ...
  pdf_document: 
    ...
    toc_depth: 6
  word_document:
    reference_docx: "C:/Users/jared.walker/OneDrive - Utah State Board of Education/FNS-905-SOP Template (no content).docx"
    highlight:
    toc: yes
    toc_depth: 6
date: <center><center>
author: <center>Jared Walker<center>
---
...
```{r message=FALSE, warning=FALSE}
## RICH JR HIGH/NRES 
all[which(all$state_site_id_number == "24-304"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "24-304"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "24-304"), grep("site_address1", colnames(all))] <- "54 E 100 S" ## Correction
all[which(all$state_site_id_number == "24-304"), grep("site_address1", colnames(all))] ## Display correction

## NORTH SUMMIT HIGH
all[which(all$state_site_id_number == "21-704"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "21-704"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "21-704"), grep("site_address1", colnames(all))] <- "53 S 100 E" ## Correction
all[which(all$state_site_id_number == "21-704"), grep("site_address1", colnames(all))] ## Display correction

## NORTH SUMMIT ELEMENTARY
all[which(all$state_site_id_number == "21-110"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "21-110"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "21-110"), grep("site_address1", colnames(all))] <- "240 S BEACON DR" ## Correction
all[which(all$state_site_id_number == "21-110"), grep("site_address1", colnames(all))] ## Display correction

## NORTH SUMMIT MIDDLE
all[which(all$state_site_id_number == "21-304"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "21-304"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "21-304"), grep("site_address1", colnames(all))] <- "76 S 100 E" ## Correction
all[which(all$state_site_id_number == "21-304"), grep("site_address1", colnames(all))] ## Display correction

## BLUE PEAK HIGH
all[which(all$state_site_id_number == "30-740"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "30-740"), grep("site_address1", colnames(all))] ## Display bad address address
all[which(all$state_site_id_number == "30-740"), grep("site_address1", colnames(all))] <- "211 South Tooele Blvd." ## Correction
all[which(all$state_site_id_number == "30-740"), grep("site_address1", colnames(all))] ## Display correction

## NAA TSIS' AAN COMMUNITY SCHOOL
all[which(all$state_site_id_number == "Q8-1"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "Q8-1"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "Q8-1"), grep("site_address1", colnames(all))] <- "24 Highway 98" ## Correction
all[which(all$state_site_id_number == "Q8-1"), grep("site_address1", colnames(all))] ## Display correction
## NAA TSIS' AAN COMMUNITY SCHOOL
all[which(all$state_site_id_number == "Q8-1"), grep("site_city", colnames(all))] ## Display bad city
all[which(all$state_site_id_number == "Q8-1"), grep("site_city", colnames(all))] <- "Tonalea" ## Correction
all[which(all$state_site_id_number == "Q8-1"), grep("site_city", colnames(all))] ## Display correction
## NAA TSIS' AAN COMMUNITY SCHOOL
all[which(all$state_site_id_number == "Q8-1"), grep("site_zip", colnames(all))] ## Display bad zip
all[which(all$state_site_id_number == "Q8-1"), grep("site_zip", colnames(all))] <- 86044 ## Correction
all[which(all$state_site_id_number == "Q8-1"), grep("site_zip", colnames(all))] ## Display correction

## GROUSE CREEK SCHOOL
all[which(all$state_site_id_number == "03-140"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "03-140"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "03-140"), grep("site_address1", colnames(all))] <- "76785 W 11900 N" ## Correction
all[which(all$state_site_id_number == "03-140"), grep("site_address1", colnames(all))] ## Display correction

## BELKNAP ELEMENTARY
all[which(all$state_site_id_number == "02-104"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "02-104"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "02-104"), grep("site_address1", colnames(all))] <- "510 North 650 East" ## Correction
all[which(all$state_site_id_number == "02-104"), grep("site_address1", colnames(all))] ## Display correction

## BEAVER HIGH
all[which(all$state_site_id_number == "02-704"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "02-704"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "02-704"), grep("site_address1", colnames(all))] <- "195 E CENTER" ## Correction
all[which(all$state_site_id_number == "02-704"), grep("site_address1", colnames(all))] ## Display correction

## WELLINGTON ELEMENTARY
all[which(all$state_site_id_number == "05-154"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "05-154"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "05-154"), grep("site_address1", colnames(all))] <- "250 W 200 N" ## Correction
all[which(all$state_site_id_number == "05-154"), grep("site_address1", colnames(all))] ## Display correction

## BLUFF ELEMENTARY 
all[which(all$state_site_id_number == "25-108"), grep("site_name", colnames(all))] ## Display site name
all[which(all$state_site_id_number == "25-108"), grep("site_address1", colnames(all))] ## Display bad address
all[which(all$state_site_id_number == "25-108"), grep("site_address1", colnames(all))] <- "Old Main Highway 191" ## Correction
all[which(all$state_site_id_number == "25-108"), grep("site_address1", colnames(all))] ## Display correction
```

# 3 - Filter Inactive Sites
